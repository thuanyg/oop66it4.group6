...
//        DashboardJDBC ds = new DashboardJDBC();
    ...
    public void setTableTopDocGia() {
        ArrayList<TopDocGiaBean> list = ds.TopDocGia();
        if (!list.isEmpty()) {
            home.getTopDocGiaTableModel().setRowCount(0);
            list.forEach((dg) -> {
                home.getTopDocGiaTableModel().addRow(new Object[]{dg.getId(), dg.getHoten(), dg.getSoL()});
            });
        }
    }
    ...
    public void ShowTienPhat() {
        ...
        if (PhieuMuonQuaHanController.optionPaneCount == 0) {
            JOptionPane.showMessageDialog(home, "Số tiền phạt của phiếu mượn mã " + maPM + " là " + tienphat + " (VND).");
            PhieuMuonQuaHanController.optionPaneCount++;
        }
    }
